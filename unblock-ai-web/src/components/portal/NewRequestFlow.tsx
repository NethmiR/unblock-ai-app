...
interface Props {
  /** From `getRequesterContext(session)` - fetched server-side in page.tsx. */
  requesterContext: Record<string, unknown>;
}

/**
 * Owns the new-request page's client state. Split out of page.tsx so the
 * page itself can stay a Server Component that reads the session cookie and
 * hands the requester context down as a prop - see useSelectionSession.ts.
 */
export function NewRequestFlow({ requesterContext }: Props) {
  ...
  const {
    ...
    rejectMatch,
    hasStarted,
  } = useSelectionSession(requesterContext);
